# library
import json
from ml_prediction.decision_tree_ml import build_tree, predict
from gui.test_sensor import read_sensor
from gui.test_pump import run_pump
...
def run_model(plantdiary, volt_threshold):
	# training data-set
	...
	humidity = print(f"Humidity value: {humidity}")
	# date stamp
	day = datetime.now().isoformat()
	time2 = time_parser(day)
	# days since last watered
	days_since_last_watered2 = ((time2 - max(watered_days)).days if watered_days else 1000)
	# append to x2
	x2.append([humidity, days_since_last_watered2, time2.hour])
	x2 = np.array(x2)
	# create prediction
	...
	if prediction == 1:
		...
		# error message
		message = print("There was an error! :(")
...
if __name__ == "__main__":
	...
	plantdiary = json.load(open(sys.argv[1]))
	volt_threshold = sys.argv[2]
	output = run_model(plantdiary, volt_threshold)
